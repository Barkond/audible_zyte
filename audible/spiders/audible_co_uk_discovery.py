...
class AudibleCoUkDiscoverySpider(scrapy.Spider):
    ...
    def parse(self, response):  # NOQA
        for top_category in response.css('.bc-row-responsive.bc-spacing-medium'
                                         ' h2 a'):
            yield scrapy.Request(url=urljoin(response.url,
                                             top_category.css('::attr(href)')),
                                 callback=self.parse_categories)

    def parse_categories(self, response):
        for category in response.css('.bc-row-responsive.'
                                     'subCategoriesContainer div'):
            if category.css('a'):
                url = response.urljoin(category.css('a::attr(href)''').get())
                yield scrapy.Request(url=url, callback=self.parse_categories)
            yield scrapy.Request(
                url=urljoin(response.url, response.css('.bc-col-responsive.'
                                                       'bc-text-right.bc-col-4'
                                                       ' a::attr(href)')),
                callback=self.parse_product_list)

    def parse_product_list(self, response):  # NOQA
        for product in response.css('div.adbl-impression-container'
                                    ' li.bc-list-item.productListItem'):
            item = {
                'publication_date': product.css('li.bc-list-item.'
                                                'releaseDateLabel'
                                                ' span::text').re_first(
                    r'(\d{2}-\d{2}-\d{2})'),
                'lang': product.css('li.bc-list-item.languageLabel'
                                    ' span::text').re_first(r'(\w*)\s*$')
            }
            ...
            if product.css('li.bc-list-item.ratingsLabel'
                           ' span.bc-text.bc-pub-offscreen'):
                item['avg_review_score'] = float(product.css(
                    'li.bc-list-item.ratingsLabel'
                    ' span.bc-text.bc-pub-offscreen::text').re_first(
                    r'^(\d\.\d|\d)', '0'))
            item_reviews_count = product.css(
                'li.bc-list-item.ratingsLabel'
                ' span.bc-text.bc-size-small::text').re_first(
                r'(\d+\,\d+|\d)', '0')
            ...
            if product.css('li.bc-list-item.subtitle'):
                subtitle = product.css('li.bc-list-item.subtitle'
                                       ' span::text').get()
            ...
            product_duration = product.css('li.bc-list-item.'
                                           'bc-list-item.runtimeLabel'
                                           ' span::text').get()
            if product_duration:
            ...
            breadcrumps = []
            for bc in response.css('ul.bc-list.bc-spacing-none'
                                   ' li.bc-list-item a'):
            ...
            last_bc = {}
            last_bc_name = response.css('div.bc-box.bc-box-padding-none'
                                        ' ul.bc-list '
                                        'li.bc-list-item span::text').get()
            if last_bc_name:
                last_bc['name'] = last_bc_name.strip()
            last_bc_url = response.xpath('//link'
            ...
            price = product.css('div.adblBuyBoxPrice'
                                ' > p.buybox-regular-price > '
                                'span:last-child::text').re_first(
                r'(\d+\.\d+)')
            currency = product.css('div.adblBuyBoxPrice > '
                                   'p.buybox-regular-price > '
                                   'span:last-child::text').re_first(
                r'^\s+(.)')
            if price:
            ...
            next_page = response.css('span.bc-button.'
                                     'bc-button-secondary.nextButton '
                                     'a::attr(href)').get()
            if response.css('span.bc-button.bc-button-secondary.'
                            'nextButton.bc-button-disabled'):
                pass
            else:
